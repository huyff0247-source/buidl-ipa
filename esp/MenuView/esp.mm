...
- (void)renderESPToLayers:(NSMutableArray<CALayer *> *)layers {
    if (Moudule_Base == -1 || Moudule_Base == 0) {
        snprintf(g_debugStatus, sizeof(g_debugStatus), "LOI: Module base=0x%llx (khong tim thay freefireth)", Moudule_Base);
        return;
    }

    uint64_t matchGame = getMatchGame(Moudule_Base);
    if (!isVaildPtr(matchGame)) {
        snprintf(g_debugStatus, sizeof(g_debugStatus), "LOI: matchGame=0x%llx (offset 0xBFD8978 sai / chua vao tran)", matchGame);
        return;
    }
    ...
    if (!isVaildPtr(player)) {
        snprintf(g_debugStatus, sizeof(g_debugStatus), "LOI: player dict=0x%llx (offset 0xC8 sai)", player);
        return;
    }
    ...
    if (!isVaildPtr(tValue) || coutValue <= 0 || coutValue > 200) {
        snprintf(g_debugStatus, sizeof(g_debugStatus), "LOI: dict tValue=0x%llx count=%d", tValue, coutValue);
        return;
    }
