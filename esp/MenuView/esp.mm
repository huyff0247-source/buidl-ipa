...
- (void)SetUpBase {
    ...
    dispatch_once(&onceToken, ^{
        ...
        NSLog(@"[ESP] SetUpBase: Moudule_Base = 0x%llx", Moudule_Base);
    });
}
...
- (void)renderESPToLayers:(NSMutableArray<CALayer *> *)layers {
    if (Moudule_Base == -1 || Moudule_Base == 0) {
        ...
        if (logCount < 3) {
            NSLog(@"[ESP] renderESPToLayers: Moudule_Base chua san sang: 0x%llx", Moudule_Base);
            logCount++;
        }
        return;
    }
    ...
    if (!isVaildPtr(matchGame)) {
        static int logCount = 0;
        if (logCount < 3) {
            NSLog(@"[ESP] renderESPToLayers: matchGame khong hop le: 0x%llx", matchGame);
            logCount++;
        }
        return;
    }
    ...
    if (!isVaildPtr(camera)) {
        static int logCount = 0;
        if (logCount < 3) {
            NSLog(@"[ESP] renderESPToLayers: camera khong hop le: 0x%llx", camera);
            logCount++;
        }
        return;
    }
    ...
    if (!isVaildPtr(match)) {
        static int logCount = 0;
        if (logCount < 3) {
            NSLog(@"[ESP] renderESPToLayers: match khong hop le: 0x%llx", match);
            logCount++;
        }
        return;
    }
    ...
    if (!isVaildPtr(myPawnObject)) {
        static int logCount = 0;
        if (logCount < 3) {
            NSLog(@"[ESP] renderESPToLayers: myPawnObject khong hop le: 0x%llx", myPawnObject);
            logCount++;
        }
        return;
    }
    ...
    if (!isVaildPtr(player)) {
        static int logCount = 0;
        if (logCount < 3) {
            NSLog(@"[ESP] renderESPToLayers: player dictionary khong hop le: 0x%llx", player);
            logCount++;
        }
        return;
    }
    ...
    if (!isVaildPtr(tValue) || coutValue <= 0 || coutValue > 200) {
        static int logCount = 0;
        if (logCount < 3) {
            NSLog(@"[ESP] renderESPToLayers: dictionary entries khong hop le: tValue=0x%llx count=%d", tValue, coutValue);
            logCount++;
        }
        return;
    }
    ...
    if (matrix == NULL) {
        static int logCount = 0;
        if (logCount < 3) {
            NSLog(@"[ESP] renderESPToLayers: view matrix NULL");
            logCount++;
        }
        return;
    }
    ...
    if (successLogCount < 3) {
        NSLog(@"[ESP] renderESPToLayers: OK matchGame=0x%llx camera=0x%llx match=0x%llx myPawn=0x%llx count=%d", 
