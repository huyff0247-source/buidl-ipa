...
- (void)updateFrame {
    ...
    // Debug status text hien thi goc tren trai man hinh
    {
        CATextLayer *dbg = [CATextLayer layer];
        dbg.string = [NSString stringWithFormat:@"%s | players=%d", g_debugStatus, g_playerCount];
        dbg.fontSize = 12;
        dbg.frame = CGRectMake(10, 40, 600, 18);
        dbg.foregroundColor = [UIColor yellowColor].CGColor;
        dbg.contentsScale = [UIScreen mainScreen].scale;
        [self.drawingLayers addObject:dbg];
    }
...
- (void)renderESPToLayers:(NSMutableArray<CALayer *> *)layers {
    ...
    // Danh sach player: match+0xC8 (cu) SAI -> do la bool DDNEPIHDBGL trong dump.
    // Danh sach that la cac field List<Player> trong EMKJHAJNPDH:
    //   0x158 CGLCNGIMJLF, 0x160 EIEKICOGNHF, 0x578 OMPOPEJDMDD
    // List<T> layout: _items(Player[])=+0x10, _size(int)=+0x18; array data=+0x20.
    static const uint64_t kListCands[] = { 0x158, 0x160, 0x578 };
    uint64_t player = 0;   // List<Player>
    uint64_t tValue = 0;   // Player[] items array
    int coutValue = 0;
    for (size_t li = 0; li < sizeof(kListCands)/sizeof(kListCands[0]); ++li) {
        ...
        if (player == 0 && sz > 0 && sz <= 100 && isVaildPtr(items)) {
            player = lp; tValue = items; coutValue = sz;
        }
    }
    if (!isVaildPtr(tValue) || coutValue <= 0 || coutValue > 200) {
        snprintf(g_debugStatus, sizeof(g_debugStatus), "LOI: player list rong (items=0x%llx count=%d)", tValue, coutValue);
        return;
    }
