...
vm_map_offset_t GetGameModule_Base(char* GameProcessName) {
    ...
    if (pid == -1) {
        NSLog(@"[ESP] GetGameModule_Base: Khong tim thay process %s", GameProcessName);
        return 0;
    }
    
    NSLog(@"[ESP] GetGameModule_Base: Tim thay pid=%d cho %s", pid, GameProcessName);
    ...
    if (get_task != MACH_PORT_NULL) {
        ...
        if (kr == KERN_SUCCESS) {
            NSLog(@"[ESP] GetGameModule_Base: Module base = 0x%llx (size=0x%llx)", vmoffset, vmsize);
            return vmoffset;
        } else {
            NSLog(@"[ESP] GetGameModule_Base: mach_vm_region_recurse that bai: %d", kr);
        }
    } else {
        NSLog(@"[ESP] GetGameModule_Base: task_for_pid that bai");
    }
