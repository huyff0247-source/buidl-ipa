...
uint64_t getMatchGame(uint64_t Moudule_Base) {
    ...
    if (!isVaildPtr(GameFacade_Static)) {
        snprintf(g_matchDiag, sizeof(g_matchDiag), "Static=0x%llx (TypeInfo ok=0x%llx, offset 0xB8 sai?)", (unsigned long long)GameFacade_Static, (unsigned long long)GameFacade_TypeInfo);
