...
uint64_t getMatchGame(uint64_t Moudule_Base) {
    ...
    NSLog(@"[ESP] getMatchGame: GameFacade_TypeInfo=0x%llx", GameFacade_TypeInfo);
    if (!isVaildPtr(GameFacade_TypeInfo)) return 0;
    
    uint64_t GameFacade_Static = ReadAddr<uint64_t>(GameFacade_TypeInfo + 0xB8);
    NSLog(@"[ESP] getMatchGame: GameFacade_Static=0x%llx", GameFacade_Static);
    if (!isVaildPtr(GameFacade_Static)) return 0;
    
    uint64_t matchGame = ReadAddr<uint64_t>(GameFacade_Static + 0x0);
    NSLog(@"[ESP] getMatchGame: matchGame=0x%llx", matchGame);
    return matchGame;
}

uint64_t getMatch(uint64_t matchgame) {
    uint64_t match = ReadAddr<uint64_t>(matchgame + 0x90);
    NSLog(@"[ESP] getMatch: match=0x%llx", match);
    return match;
}
...
uint64_t getLocalPlayer(uint64_t match) {
    ...
    NSLog(@"[ESP] getLocalPlayer: localPlayer=0x%llx", localPlayer);
    return localPlayer;
}

uint64_t CameraMain(uint64_t matchgame) {
    uint64_t CameraControllerManager = ReadAddr<uint64_t>(matchgame + 0xD8);
    NSLog(@"[ESP] CameraMain: CameraControllerManager=0x%llx", CameraControllerManager);
